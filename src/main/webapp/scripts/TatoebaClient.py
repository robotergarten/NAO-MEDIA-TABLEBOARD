...
def get_sentences_for_word(word, src_lang = 'und', dest_lang = 'und', numberSentences = 'und'):
    maxLenghtSentence = 25;
    ...
    for link in sentence_links:
        ...
        if len(sentences) < int(numberSentences):
...
def get_command_line_parser():
    ...
    parser.add_argument('-a', '--dest-lang2', default='und')
    parser.add_argument('-n', '--numberSentences', default='und')
    parser.add_argument('-f', '--outputFile', default='und')
    parser.add_argument('-i', '--infile')
    return parser
    
def output_sentences(sentences, outputFile):
    ...
    for sentence in sentences:
        ...
        satze = unicode(sentence['main_lang'] + ': ' + sentence['main']+'\r\n')
        file.write(satze.encode('utf8'))
        
        ubersetzung =''
        
        for t in sentence['translations']:
            #print smart_str('\t' + t['lang'] + ':\t' + t['translation'])
            ubersetzung = unicode(t['lang'] + ':' + t['translation']+'\r\n')
        file.write(ubersetzung.encode('utf8'))
        file.write('---' + '\r\n')
...
if __name__ == "__main__":
    ...
    if args['word'] != None:
        sentences = get_sentences_for_word(args['word'], args['src_lang'], args['dest_lang'], args['numberSentences'])
        
        if len(sentences) == 0:
            sentences = get_sentences_for_word(args['word'], args['src_lang'],args['dest_lang2'], args['numberSentences'])
        
        output_sentences(sentences, args['outputFile'])
    elif args['infile'] != None:
        words = get_words_from_file(args['infile'])
        for word in words:
            sentences = get_sentences_for_word(word, args['src_lang'], args['dest_lang'],args['dest_lang2'])
            output_sentences(sentences, args['outputFile'])            
    else:
        parser.print_help()        
